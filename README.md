...
The bundled build links macOS system libraries (zlib, bzip2, iconv) and the vendored zstd static
library to keep the app self-contained. Formats requiring xz/lzma, lz4, or blake2 are not available
in the bundled build; install `fuse-archive` via Homebrew and Rugzak will use it automatically as a
fallback.
...
| Compression filters `xz`, `lz4`, `lzma`, `lzo`, `br`, `lrz`, `grz`    | 🔲      | ✅                           |
...
- The bundled `fuse-archive` omits xz/lzma, lz4, and blake2 (RAR5 checksums). Install
