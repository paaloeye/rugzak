...
`vendor_init.sh` clones `fuse-archive`, `lz4`, and `libarchive` into `vendor/` at their pinned commits.
...
The bundled build links macOS system libraries (zlib, bzip2, iconv) and a vendored static lz4 to
keep the app self-contained. Formats requiring xz/lzma, zstd, or blake2 are not available in the
bundled build; install `fuse-archive` via Homebrew and Rugzak will use it automatically as a fallback.
...
| Compression filters `xz`, `zst`, `lzma`, `lzo`, `br`, `lrz`, `grz`    | 🔲      | ✅                           |
...
- The bundled `fuse-archive` omits xz/lzma, zstd, and blake2 (RAR5 checksums). Install
