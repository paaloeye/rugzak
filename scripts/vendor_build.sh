...
build_fuse_archive_arch() {
    ...
    make -C "${FUSE_SRC}" \
        PKG_CXXFLAGS="${FUSE3_CFLAGS} -I${ARCHIVE_INC} -I${LZMA_INC} -I${LIBB2_INC} -I${ZSTD_INC} -I${LZ4_INC}" \
        PKG_LDFLAGS="${FUSE3_LIBS} ${ARCHIVE_A} ${LZMA_A} ${LIBB2_A} ${ZSTD_A} ${LZ4_A} ${COMP_LIBS}" \
        CXXFLAGS="${ARCH_FLAGS}" \
        LDFLAGS="${ARCH_FLAGS}" \
        -j"${NCPU}" \
